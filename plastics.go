...
func calcCIMPlstic(ctx *config.SpiceContext, dbx *sqlx.DB, mastPlan model.MasterPlan) ([]model.MOIPlastic, error) {
	plasticRec := map[string]model.PlsticRec{}
	cardMdl := map[string]model.CardModule{}
	mstrPlnOfr := map[string]model.MasterPlanOffer{}
	cache := []int{}
    cache = append(plasticRec,cardMdl...)
    cache = append(cache,mstrPlnOfr...)
	plastics := make([]model.MOIPlastic, len(mastPlan.Offers))
